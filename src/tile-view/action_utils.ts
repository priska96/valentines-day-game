...
import {CharacterState, UpdatePlayerPositionAction} from "./character/slices/characterSlice";
...
export const enterDungeon = (
    ...
    if(action === 'enter-dungeon'){
        setContents({open: false, title: '', text: '', openerId: '', action: ''});
        changeMap('evilKing');
        updateObject({idx:[5,6],'data-5':{ x:4 ,y: 10},'data-6':{ x:4 ,y: 11}})
...
export const victory = (
    ...
    if(action === 'victory'){
        ...
        onGameEnd({mode: 'get-out', winner: 'Jihoon', selectedOpponentIdx: otherThingIdx})
        return true;
    }
    return false;
}
...
export const exitDungeon = (
    ...
    if(action === 'exit-dungeon'){
        ...
        updateNPC({idx:[2],'data-2':{ x:4 ,y: 3, step: 0, dir: 0, followHero:true, stopMoving:false, map:['sky', 'evilKing', 'dungeonPath']}});
...
export const leaveDungeon = (
    ...
    if(action === 'leave-dungeon'){
        ...
        onGameEnd({mode: 'get-out', winner: 'Jihoon', selectedOpponentIdx: 0})
        updateNPC({idx:[2],'data-2':{ x:6 ,y: 7, step: 0, dir: 0, stopMoving: true, map:['skyBroken']}});
        updateNPC({idx:[0],'data-0':{ map:['skyBroken']}});
...
export const goBackToGround = (
    ...
    if(action === 'go-to-ground'){
        ...
        updateNPC({idx:[0],'data-0':{ x:15 ,y: 6, step: 0, dir: 3, stopMoving:false, map:['forest']}})
        updateNPC({idx:[2],'data-2':{ x:14,y: 7, step: 0, dir: 3, stopMoving:false, map:['forest'], followHero: false}})
...
export const followHeroHome = (
    ...
    if(action === 'follow-hero-home'){
        setContents({open: false, title: '', text: '', openerId: '', action: ''});
        updateNPC({idx:[2],'data-2':{x: character.x, y:character.y-1, dir: 0, followHero: true}})
        return true
    }
    return false;
}
...
export const goToForest2 = (
    ...
    changeMap('forest2');
    updateNPC({idx:[2],'data-2':{ x:15,y: 0, step: 0, dir: 0, stopMoving:false, map:['forest2'], followHero: true}})
...
export const goToForest3 = (
    ...
    changeMap('forest3');
    updateNPC({idx:[2],'data-2':{ x:15,y: 0, step: 0, dir: 0, stopMoving:false, map:['forest3'], followHero: true}})
    updatePlayerPosition({x:15, y: 1, step: 0, dir: 0})
}
export const goToForest4 = (
    ...
    changeMap('forest4');
    updateNPC({idx:[2],'data-2':{ x:0,y: 10, step: 0, dir: 2, stopMoving:false, map:['forest4'], followHero: true}})
...
export const  goToPiscesTown = (
    ...
    updateNPC({idx:[2],'data-2':{ x:14,y: 15, step: 0, dir: 3, stopMoving:false, map:['piscesTown'], followHero: true}})
    updatePlayerPosition({x:14, y: 14, step: 1, dir: 3})
    setTimeout(() => {
            setContents(dialogs.piscesTown['npc-2'].enterTown.content)
        }, 500
    )
}
export const goToPiscesTown2 = (
    ...
    updateNPC({idx:[2],'data-2':{ x:0,y: 7, step: 0, dir: 2, stopMoving:false, map:['piscesTown2'], followHero: true}})
...
export const goToPiscesTownFrom2 = (
    ...
    changeMap('piscesTown');
    updateNPC({idx:[2],'data-2':{ x:16,y: 7, step: 1, dir: 1, stopMoving:false, map:['piscesTown'], followHero: true}})
...
export const goToPiscesTown3 = (
    ...
    updateNPC({idx:[4,3, 2],
        ...
        'data-2':{ x:8,y: 15, step: 1, dir: 3, stopMoving:true, map:['piscesTown3'], followHero: false
    }})
    updatePlayerPosition({x:8, y: 14, step: 1, dir: 3});
    setTimeout(()=> {
            setContents(dialogs.piscesTown["npc-3"].beforeFight.content)
        },200
    )
}
...
export const beforeBattleEvilQueen = (
    ...
    if(action === 'before-battle-evil-queen'){
        setContents({open: false, title: '', text: '', openerId: '', action: ''});
        updateNPC({idx:[2],'data-2':{animate:"walk-to-dad", stopMoving:true} })
        return true
    }
    return false
}
...
export const beforeBattleEvilQueen2 = (
    ...
    if(action === 'before-battle-evil-queen2'){
        ...
        updateNPC({
            idx:[2,1],
            'data-2':{ x:7,y: 12},
            'data-1':{ x:8,y: 0, step: 1, dir: 1, stopMoving:true, map:['piscesTown3'],
                followHero: false, animate:"evil-king-fall-down"}
        })
        return true
    }
    return false
...
export const victoryEvilQueen = (
    ...
    if(action === 'victory-evil-queen'){
        ...
        onGameEnd({mode: 'victory-evil-queen', winner: 'Jihoon', selectedOpponentIdx: otherThingIdx})
        return true;
    }
    return false;
}
...
export const gameOverEvilQueen = (
    ...
    if(action === 'game-over-evil-queen'){
        setContents({open: false, title: '', text: '', openerId: '', action: ''});
        onGameEnd({mode: 'game-over', winner: 'Evil Queen', selectedOpponentIdx: otherThingIdx})
        return true;
    }
    return false;
}
...
export const spellBroken = (
    ...
    if(action === 'spell-broken'){
        ...
        updateNPC({idx:[6,5, 4, 3,2,1 ],
            'data-6':{ x: 13, y: 6, map:['piscesTown3Melted'], stopMoving:false},
            'data-5':{ x: 3, y: 12, map:['piscesTown3Melted'], stopMoving:false},
